...
$._farbtastic = function (container, options) {
  ...
  fb.widgetCoords = function (event) {
    if (event.originalEvent.changedTouches[0]) {
      var event = event.originalEvent.changedTouches[0]; // le premier doigt
      return {
  ...
  fb.mousedown = function (event) {
    ...
    if (!$._farbtastic.dragging) {
      $(document).bind('mousemove touchmove', fb.mousemove).bind('mouseup touchend', fb.mouseup);
  ...
  fb.mouseup = function () {
    // Uncapture mouse
    $(document).unbind('mousemove touchstart', fb.mousemove);
    $(document).unbind('mouseup touchend', fb.mouseup);
    $(document).unbind('mousedown touchstart', fb.mousedown);
  ...
  $('canvas.farbtastic-overlay', container).bind('mousedown touchstart' , fb.mousedown);
